...
<div class="m-portlet m-portlet--mobile" >
            ...
            <div class="m-portlet__head-tools">
                    <ul class="m-portlet__nav">
                        {{-- <li class="m-portlet__nav-item">
                            <a href="{{ route('dashboard.order.ta3med.index') }}" class="btn btn-primary m-btn m-btn--pill m-btn--custom m-btn--icon m-btn--air">
                                <span>
                                    <i class="fa fa-stream"></i>
                                    <span>عرض جميع طلبات التعميد</span>
                                </span>
                            </a>
                        </li>
    ...
    <form method="POST" action="{{ route('dashboard.order.ta3med.update', ['id' => $order->id]) }}" class="m-form m-form--label-align-left- m-form--state-" id="m_form">
        @csrf
        @method('PUT')
        <!--begin: Form Body -->
                ...
                <div class="col-xl-8 offset-xl-2">
                    <div class="m-form__section m-form__section--first">
                        ...
                        {{-- @if ($order->type == 1 && $order->payment_method == 1 ) --}}
                            ...
                            <div class="form-group m-form__group row">
                                <label class="col-xl-3 col-lg-3 col-form-label">جوال صاحب الطلب:</label>
                                <div class="col-xl-9 col-lg-9">
                                    <input type="text"  class="form-control m-input"  value="{{ $order->phone_client }}" disabled>
                                </div>
                            </div>
                            <div class="form-group m-form__group row">
                                <label class="col-xl-3 col-lg-3 col-form-label">جوال المعقب	:</label>
                                <div class="col-xl-9 col-lg-9">
                                    <input type="text" class="form-control m-input" value="{{$order->phone_mo3akeb}}" disabled>
                                </div>
                            </div>
                            <div class="form-group m-form__group row">
                                ...
                                <div class="form-group m-form__group row">
                                    ...
                                    @foreach($images as $img)
                                        ...
                                        {{-- data-target="#lightBox" data-toggle="modal" --}}
                                            <img data-url="{{ asset('storage/order_ta3med') }}/{{$img}}" class="openImageModal" style="display: inline-block;
                                            float: right;
                                            ...
                                            cursor:pointer;
                                            margin-left: 5px;" 
                                            src="{{ asset('storage/order_ta3med') }}/{{$img}}" style="margin-left:7px;" width="50" height="50">
                            ...
                            @if($order->payment_method != "online_payment")
                                <div class="form-group m-form__group row">
                                    ...
                                    @elseif($order->prove_status == 0 && $order->transfer_prove != 0)
                                        ...
                                        <div class="col-xl-4 col-lg-4" >
                                            {{-- Approve--}}
                                            <a onclick="event.preventDefault(); if(confirm('هل انت متأكد من تأكيد عملية الدفع؟')) { document.getElementById('approvePayment').submit(); }else{ return false }" href="{{ route('dashboard.order.ta3med.paymentStatus', ['id' => $order->id ]) }}"  class="btn btn-success">تأكيد الدفع</a>
                                        
                                            {{-- Deny--}}
                                            <a onclick="event.preventDefault(); if(confirm('هل انت متأكد من رفض عملية الدفع؟')) { document.getElementById('denyPayment').submit();}else{ return false }" href="{{ route('dashboard.order.ta3med.paymentStatus', ['id' => $order->id ]) }}" class="btn btn-outline-danger">رفض الدفع</a>
                                        </div>
...
<div></div>
 ...
 <form id="denyPayment" action="{{ route('dashboard.order.ta3med.paymentStatus', ['id' => $order->id ]) }}" method="POST" style="display: none;">
    @method('PUT')
    @csrf
    <input type="hidden" name="ta3medOrderId" value="{{ $order->id }}">
    <input type="hidden" name="paymentStatus" value="-1">
</form>
<form id="approvePayment" action="{{ route('dashboard.order.ta3med.paymentStatus', ['id' => $order->id ]) }}" method="POST" style="display: none;">
    @method('PUT')
    @csrf
    <input type="hidden" name="ta3medOrderId" value="{{ $order->id }}">
    <input type="hidden" name="paymentStatus" value="1">
  ...
  <div class="modal-dialog modal-lg" role="document">
    <div class="modal-content">
        ...
        <!-- Carousel markup goes in the modal body -->
            <form id="changeStatus" action="{{ route('dashboard.order.ta3med.changeStatus', ['id' => $order->id ]) }}" method="POST" style="display: block;">
                @method('PUT')
                @csrf
