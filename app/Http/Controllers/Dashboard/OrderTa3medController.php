...
class OrderTa3medController extends Controller
{
    ...
    public function index()
    {
        ...
        $city = City::get();
        $bank = Bank::get();
        $orders = Order_ta3med::latest()->paginate($setting->paginate);
        return view('dashboard.orders.index_ta3med', ['orders' => $orders,'statuses' => $statuses, 'city'=>$city, 'bank'=>$bank]);
    }
    ...
    public function showTa3med($id)
    {
        ...
        if(isset($order)){
            ...
            $setting = Setting::first();
            $orders = Order_ta3med::latest()->paginate($setting->paginate);
            return view('dashboard.orders.index_ta3med', ['orders' => $orders,'statuses' => $statuses]);
    ...
    public function paymentStatusTa3med(Request $r, $id)
    {
        ...
        $r->validate([
            'ta3medOrderId' => ['required'],
            'paymentStatus' => ['required'],
        ]);

        if($r->ta3medOrderId ==  $id){
