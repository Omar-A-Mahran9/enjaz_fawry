...
class OrderTa3medController extends Controller
{
    ...
    public function ta3med()
    {
        $setting = Setting::get()->first();
        $user = Auth::user();
        $terms = Terms::get()->first();
        $banks = Bank::all();
        return view('engazFawry.form2', [
            'pagetitle' => __('engazFawry.form2'),
            'banks' => $banks,
            'setting' => $setting,
            'terms' => $terms,
            'user' => $user,
        ...
        if( $r->hasFile('uploadFile')){
         ...
         //   dd($fileIdNameNew );
           ...
           /*  $fileExt            = $r->uploadFile->getClientOriginalExtension();
            $fileIdNameNew        = uniqid() . time() . '.' . $fileExt;
            $r->uploadFile->storeAs('public/order_ta3med', $fileIdNameNew); */
    ...
    public function index()
    {
        ...
        $order_ta3med = $user->ta3med_orders;
        $order_ta3med_count = $user->ta3med_orders->count();

        return view('engazFawry.dashboard.index_ta3med', [
    ...
    public function ta3medShow(Request $r,$oid)
    {
        $user = User::find(auth()->user()->id);
        ...
        if($response){
             ...
             return view('engazFawry.dashboard.order_ta3med', [
                'pagetitle' => __('order_ta3med'),
                'order' =>  $order ,
                'user' =>  $user ,
                'response' =>  $response,
            ]);
    ...
    public function ta3medProve(Request $r)
    {
    ...
    public function printInvoice($id)
    {
        ...
        if( in_array($id, $orders) ){
            ...
            $pdf = PDF::loadView('engazFawry.dashboard.printInvoice_ta3med', array('order' => $order, 'tPrice' => $tPrice, 'mobile' => $mobile), []);
