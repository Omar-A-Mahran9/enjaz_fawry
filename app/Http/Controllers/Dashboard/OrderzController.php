...
class OrderzController extends Controller
{
    ...
    public function index()
    {
        ...
        $orders = Order::latest()->paginate($setting->paginate);
        return view('dashboard.orders.index', ['orders' => $orders, 'statuses' => $statuses]);
    }


    public function indexShrkat()
    {
        // if(!auth()->user()->hasAnyPermission(['order_index'])){
        //     return redirect()->back()->with('success', 'عفواً لا تملك الصلاحية');
        // }
    ...
    public function show($id)
    {
        ...
        return view('dashboard.orders.show', ['order' => $order, 'statuses' => $statuses, 'order_cars' => $order_cars]);
    }
    ...
    public function destroy($id)
    {
        ...
        $order = Order::find($id);
        $order->delete();
        return redirect()->route('dashboard.order.index')->with('success', 'تم حذف الطلب بنجاح');
    }
