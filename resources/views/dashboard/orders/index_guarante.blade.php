...
<div class="m-portlet m-portlet--mobile">
            ...
            <tbody>
               ...
               <tr @if($order->status == -1) style="color:brown" @endif>
                   ...
                   <td>
                    ...
                    <td>
                        <a href="{{ route('dashboard.order.guarante.show', ['id' => $order->id ]) }}" class="m-portlet__nav-link btn m-btn m-btn--hover-order m-btn--icon m-btn--icon-only m-btn--pill" title="عرض">
                            <i class="fa fa-eye"></i>
                        </a>
                        <a onclick="event.preventDefault(); document.getElementById('delete_form_{{$index}}').submit();" href="{{ route('dashboard.order.guarante.destroy', ['id' => $order->id ]) }}" class="m-portlet__nav-link btn m-btn m-btn--hover-order m-btn--icon m-btn--icon-only m-btn--pill" title="حذف">
                            <i class="fa fa-window-close"></i>
                        </a>
                        <form id="delete_form_{{$index}}" action="{{ route('dashboard.order.guarante.destroy', ['id' => $order->id ]) }}" method="POST" style="display: none;">
                            @method("DELETE")
                            {{-- {{ method_field('DELETE') }} --}}
                            @csrf
                        </form>
    ...
    <form method="POST" action="{{route('dashboard.order.guarante.statusUpdate')}}" class="m-form m-form--label-align-left- m-form--state- " id="m_form">
        @csrf
        @method('POST')
        <!--begin: Form Body -->
        <div class="m-portlet__body">
